...
| Host command execution via prompt-injected repo content (hostile README/comment/issue body steering the agent to run `rm -rf ~`) | `--allowedTools` drops `Bash`/`WebFetch` by default; `--permission-mode default` (not `acceptEdits`) so tool calls aren't auto-approved. |
...
|----------|-----|-----------------|
| **Prompt injection in reviewer prompts.** A diff that contains `"]}\nIgnore previous instructions..."` can steer a reviewer model to emit empty findings. | Diffs and repo contents are passed verbatim to the LLM for review. Structured mitigation would require either a reliable untrusted-content delimiter (doesn't exist in current LLM APIs) or content-filtering that would itself be steerable. | Lean on model diversity: Tier A requires three distinct models, each with different training. A prompt that silences one reviewer is unlikely to silence all three identically. Watch the `degraded_model_diversity` flag on evidence packets. Do not ship Tier A changes without three real providers installed. |
