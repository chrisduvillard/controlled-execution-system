...
- **Runtime boundary disclosure** — Codex defaults to `workspace-write`; CES records this as workspace-scoped but not manifest-tool-allowlist-enforced, because the Codex adapter does not enforce `TaskManifest.allowed_tools`. Operators can explicitly set `CES_CODEX_SANDBOX=read-only` or, with the two-key override, `danger-full-access`; invalid explicit sandbox values fail closed to `read-only` instead of expanding back to full-host access.
...
| Tampering with the audit ledger after the fact | HMAC-SHA256 chain with timing-safe comparison; forged entries fail `ces audit verify` and internal ledger integrity verification. |
| Host command execution via prompt-injected repo content (hostile README/comment/issue body steering the agent to run `rm -rf ~`) | Runtime prompt bodies are delivered over stdin instead of subprocess argv. Claude runs with `--allowedTools` dropping `Bash`/`WebFetch` by default and `--permission-mode default`; Codex defaults to `workspace-write` unless `CES_CODEX_SANDBOX` explicitly selects `read-only` or `danger-full-access`. CES still requires explicit runtime side-effect acceptance, persists the workspace delta, and surfaces runtime-boundary disclosures. Invalid explicit sandbox values fail closed to `read-only`. |
| Secret exfiltration via LLM output (agent reads `.env` and echoes it) | Regex scrubber (`scrub_secrets_from_text`) redacts known token prefixes, JWTs, credential-bearing URLs, private key material, Google service-account private keys, and `KEY=VALUE` assignments before persisting runtime output. CI also runs gitleaks using `.gitleaks.toml`. |
