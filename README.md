...
`ces proof` is the approval checkpoint. It reports `proven`, `partially_proven`, `unproven`, or `contradicted`, and keeps the recommendation at no-ship unless fresh verification matches the current completion contract and required evidence is present. For contract-bound work, `ces approve` now fails closed unless proof is `proven` and approval safety is `safe-to-review`.
...
```bash
uv tool install controlled-execution-system==0.1.28
```
...
PyPI publishing is tag-driven. Pushing to `master` runs CI only; pushing a `v*` tag such as `v0.1.28` triggers `.github/workflows/publish.yml`, which runs tests, builds distributions, smoke-tests the installed CLI, validates tag/version agreement, and publishes through trusted publishing. Follow [docs/RELEASE.md](https://github.com/chrisduvillard/controlled-execution-system/blob/master/docs/RELEASE.md) for the maintainer checklist.
