...
|----------|----------|---------|-------------|
| `CES_AUDIT_HMAC_SECRET` | Optional override | `.ces/keys/audit.hmac` | Override for local audit chain integrity in managed environments. Local projects normally use the file generated by `ces init`. |
...
- [ ] `CES_AUDIT_HMAC_SECRET` is set deliberately only when a managed environment overrides `.ces/keys/audit.hmac`
