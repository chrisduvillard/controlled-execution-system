...
| Compatibility Test Infrastructure | Generic / Compatibility | Keep optional SQLAlchemy/Postgres/Alembic coverage separate from supported local-first runtime. | compat-tests, Alembic, SQL compatibility | `tests/integration/_compat/`; `alembic.ini`; `pyproject.toml` `ci` dependency group | Optional CI/dev test invocations | Test-only DB schema/migrations and compatibility repositories | integration fixtures; CI dependency group | Fact |
