...
| You want the next safest production-readiness step and an agent-ready prompt | `builder-first` | `ces next`, `ces deliberate`, and `ces next-prompt` explain the next maturity target, generate an Approach Decision Brief with implementation/maintainer/risk perspectives and preserved dissent, optionally add `ces deliberate --challenge` domain-language challenge, then compile a bounded handoff without running an agent |
